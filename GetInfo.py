...
class MainWindow(QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.bLogin = True
        #---------------------------
        self.linkInfoHardware = 'http://10.0.0.1/hardware.php'
    ...
    def loadFinishedHandler(self):
        ...
        if self.bLogin:
            ...
            if str(self.browser.url()) == 'http://10.0.0.1/index.php':
                print('-Result:FAILED TO GO LOGIN LINK')
                self.close()
            self.navigate_login()
            self.bLogin = False
            self.bGoInfo = True
        elif self.bGoInfo:
            if str(self.browser.url()) == 'http://10.0.0.1/at_a_glance.php':
                print('-Result:FAILED TO LOGIN')
                self.close()
            self.GoInfo()
            self.bGoInfo = False
            self.bGoGetInfo = True
        elif self.bGoGetInfo:
            self.GoGetInfo()
            self.bGoGetInfo = False
            self.close()

    def Open_ClientInfo(self, idata=''):
        HOST = socket.gethostname()  # The server's hostname or IP address
        PORT = 65432  # The port used by the server
        #PC_IP = INI_r_w.Read_Value(ini_cfg_path, 'COMMON', 'PC_IP', 'str')
        while True:
            try:
                with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                    s.connect((HOST, PORT))
                    print('start')
                    s.sendall(bytes(idata, encoding="utf-8"))
                    #Rdata = str(s.recv(1024), 'utf-8')
                    #print(Rdata)                    
                    s.close()
                    print('end')
                    break
    ...
    def GoInfo(self):
        self.browser.setUrl(QUrl(self.linkInfoHardware))

    def GoGetInfo(self):
        print('------------------------')
        #self.browser.page().runJavaScript("document.getElementById('content')", self.store_value)
        current_page = self.browser.page()
        current_page.toHtml(self.store_html)
        loop = QEventLoop()
        self.toHtmlFinished.connect(loop.quit)
        loop.exec_()
        content = self.GetContent(self.html,"<span class=\"readonlyLabel\">Serial Number:</span> <span class=\"value\">","</span>").strip()
        #content = self.GetContent(self.html,"grdPRList.Data = ","grdPRList.Levels")\
        #    .strip('\n').replace(';','').replace('[[,','[[').replace('new Date','').replace(',,',',').replace(',[,',',[').strip()
        print('-Serial Number:'+str(content)+'^-^')
        #print(type(self.html))
        #lcontent = literal_eval(content)
        #self.Open_ClientInfo(content)
