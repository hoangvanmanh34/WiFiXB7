...
class MainWindow(QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        #---------------------------
        self.bLogin = True
        self.bGoRTDLink = False
        self.bGoRTD = False
        self.bRTDDone = False
        self.bRTDCheckLogOut = False
        self.bRTDCheckSuccess = False
        self.bRTDSuccess = False
        self.iRTDTime = 0
        #---------------------------
    ...
    def loadFinishedHandler(self):
        ...
        #"PyQt5.QtCore.QUrl('http://10.0.0.1/home_loggedout.php')"
        if self.bLogin:
            print("navigate_login")
            self.navigate_login()
            self.bLogin = False
            self.bGoRTDLink = True
        elif self.bGoRTDLink:
            self.GoRTDLink()
            self.bGoRTDLink = False
            self.bGoRTD = True
        elif self.bGoRTD:
            ...
            if str(self.browser.url()) == "PyQt5.QtCore.QUrl('http://10.0.0.1/home_loggedout.php')":
            #self.GoRTD()
                print("-iResult:LOGOUT_RTD^-^")
                self.iStartRTDTime = datetime.now()
                print(self.iStartRTDTime)
                self.bRTDCheckLogOut = True
                self.close()
            elif self.bRTDCheckLogOut:
                self.iEndRTDTime = (datetime.now() - self.iStartRTDTime).total_seconds()
                print(self.iEndRTDTime)
                self.close()
            '''    if self.iEndRTDTime > 90:
                    self.browser.setUrl(QUrl("http://10.0.0.1/"))#PyQt5.QtCore.QUrl('http://10.0.0.1/captiveportal.php')
                    print("-iResult:CHECK_DONE_RTD^-^")
                    self.bRTDCheckLogOut = False
                    self.bRTDCheckSuccess = True                    
            elif self.bRTDCheckSuccess:
                if str(self.browser.url()) == "PyQt5.QtCore.QUrl('http://10.0.0.1/captiveportal.php')":
                    print("-iResult:PASS_RTD^-^")                
                    self.bRTDCheckSuccess = False
                    self.bRTDDone = False
                    #self.close()'''
    ...
    def GoRTDLink(self):
        self.browser.setUrl(QUrl("http://10.0.0.1/restore_reboot.php"))

    def GoRTD(self):
        self.browser.page().runJavaScript("document.getElementById('btn5').click()", self.store_value)      
        pyautogui.press('tab')
        pyautogui.press('tab')
        pyautogui.press('enter')
