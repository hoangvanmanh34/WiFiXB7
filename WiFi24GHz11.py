...
class MainWindow(QMainWindow):
    ...
    def loadFinishedHandler(self):
        ...
        elif self.bGo2GChange:
            self.Go2GHzSecurity()
            self.bGo2GChange = False
            self.b2GChange = True
        elif not self.bGo2GChange and self.b2GChange:
            self.change2GHzSecurity()
            self.change2GHzChannel_11()
            self.SaveSetting()
            self.b2GChange = False            
            self.bClose = True
            #print('aaaaaaaaaaaa')
        elif not self.b2GChange and self.bClose:
            self.close()
    ...
    def change2GHzChannel_11(self):
        print('------------------------')
        self.browser.page().runJavaScript("document.getElementById('channel_manual').click()", self.store_value)
        #self.browser.page().runJavaScript("showDialog()", self.store_value)
        self.browser.page().runJavaScript("document.getElementById('channel_number').value = '11'", self.store_value)
        #self.browser.page().runJavaScript("document.getElementById('pop_ok').click()", self.store_value)     
