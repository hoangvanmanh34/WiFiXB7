...
class MainWindow(QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.browser.setUrl(QUrl("http://10.0.0.1/captiveportal.php?lang=eng"))
    ...
    def loadFinishedHandler(self):
        ...
        print('ABC')
        #print(self.browser.page().runJavaScript("document.getElementById('wifi-config')", self.store_value))
        #print(self.value)
        #print('ABC')
        if self.bMain:
            print('get_set_up')
            if str(self.browser.url()) == "PyQt5.QtCore.QUrl('http://10.0.0.1/index.php')":
                print('-Result:GO_INDEX_LINK^-^')
                #print(self.value)
                self.close()
            elif str(self.browser.url()) == "PyQt5.QtCore.QUrl('http://10.0.0.1/captiveportal.php?lang=eng')":
                print('start get_set_up')
                self.get_set_up()
                print('-Result:PASS_SETUP^-^')
            '''else:
                print('-Result:FAILED_SETUP')'''
        #elif not self.bMain and self.bLoginFirst:
        #    print('-Result:PASS SETUP')
        #    self.close()
        '''elif not self.bMain and self.bReloadFirst:
            print('reload')
            #time.sleep()
            self.browser.reload()
            self.bReloadFirst = False
            self.bLoginFirst = True            
        elif not self.bReloadFirst and self.bLoginFirst:
            print('main run')
            print('navigate_login first')
            self.navigate_login_First()
            self.bLoginFirst = False
        elif str(self.browser.url()) == "PyQt5.QtCore.QUrl('http://10.0.0.1/check.php')":
            if self.bHasChanged:
                print('Has changed')
            else:
                print("go admin_password_change")
                self.browser.setUrl(QUrl("http://10.0.0.1/admin_password_change.php"))
            self.bPasswordChange = True
        elif self.bPasswordChange:
            self.changePassword()
            self.bHasChanged = True
            self.browser.reload()
            self.browser.setUrl(QUrl("http://10.0.0.1/index.php"))
            print("go index")
            self.bPasswordChange = False
            self.bLogin = True
            print('-Result:PASS SETUP')
            self.close()'''
        '''elif not self.bPasswordChange and self.bLogin:
            print("navigate_login")
            self.navigate_login()
            self.bLogin = False
            self.bGo2GChange = True
        elif not self.bLogin and self.bGo2GChange:
            self.Go2GHzSecurity()
            self.bGo2GChange = False
            self.b2GChange = True
        elif not self.bGo2GChange and self.b2GChange:
    ...
    def LoadDUTlink(self):
        self.browser.setUrl(QUrl("http://10.0.0.1/"))

    def Go_First():
        self.browser.setUrl(QUrl("http://10.0.0.1/captiveportal.php"))
